...
// change this part!
		...
		// Approach - 1 : dealloc every other chunk
		for (size_t j = 0; j < CHUNK; j += 4096) {
			if (rand() % 2 == 0) {
		    		madvise((char *)data[i] + j, 4096, MADV_DONTNEED);
			}
...
//==================================================================================
	...
	//Approach 2 - random allocation of memory
	for (i = 0; i < numchunk; i++) {
		if (rand() % 2 == 0) {
		    madvise(data[i], CHUNK, MADV_DONTNEED);
		}
    	}
    	*/
